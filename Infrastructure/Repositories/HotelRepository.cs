...
namespace Infrastructure.Repositories
{
    public class HotelRepository : IHotelRepository
    {
            ...
            )
        {
            ...
            if (aminites.Length > 0) query = query.Where(am => am.HotelAmenity.Count > 0 && aminites.All(p => am.HotelAmenity.Any(o => o.AmenityId == p)));

            if (maxPrice is not null) query = query.Where(p => p.PricePerNight <= maxPrice);

            if (!String.IsNullOrEmpty(city)) query = query.Where(c => c.City.Name.Contains(city));

            if (!String.IsNullOrEmpty(country)) query = query.Where(c => c.City.Country.Contains(country));

            if (hotelType is not null && hotelType.Length > 0 ) query = query.Where(ht => hotelType.Contains(ht.HotelType));
            
            if (!String.IsNullOrEmpty(owner)) query = query.Where(o => o.Owner.Contains(owner));
            
            if (!String.IsNullOrEmpty(hotelName)) query = query.Where(o => o.Name.Contains(hotelName));
